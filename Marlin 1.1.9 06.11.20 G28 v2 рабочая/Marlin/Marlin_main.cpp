...
#define TOUCH_SIGNAL_PIN 18  // purple and white
...
uint8_t marlin_debug_flags = DEBUG_NONE;
    ...
    static int32_t position(const AxisEnum axis);//Родион
...
// Этот код нужен для M401. Поднимает соленоид (щуп). Миша 16.10.20
...
// Миша 06.11.20 Функция движения кареток. Двигает любой из 4 кареток в отдельности по осям.

static void axisRoboMove(float A, float B, float C, float D){
...
// Родион 02.11.20 Делаем свою ф-цию парковки У

static void homeaxisY(const AxisEnum axis) {
  
  bool endstopX = (digitalRead(3) < 1);
  bool endstopY = (digitalRead(14) < 1);

  SERIAL_PROTOCOLPAIR(" endstopX", endstopX);
  SERIAL_PROTOCOLPAIR(" endstopY", endstopY);

  if(endstopX){
    // Двигаем каретку У. Каретка Х уже припаркована.
    while (!endstopY){
      axisRoboMove(0.01, -0.01, 0.0, 0.0);
      endstopY = (digitalRead(14) < 1);
      }
      axisRoboMove(-4.0, 4.0, 0.0, 0.0);
      SERIAL_PROTOCOLPAIR("endstopY: ", endstopY);
      endstopY = (digitalRead(14) < 1);
    }else if(endstopY) {
      // Двигаем каретку Х. Каретка У уже припаркована.
      while (!endstopX){
      axisRoboMove(0.0, 0.0, -0.01, -0.01);
      endstopX = (digitalRead(3) < 1);
      }
      axisRoboMove(0.0, 0.0, 4.0, 4.0);
      SERIAL_PROTOCOLPAIR("endstopX: ", endstopX);
      endstopX = (digitalRead(3) < 1);
      }
...
static void homeaxis(const AxisEnum axis) {//Родион заметка
  if (axis == Z_AXIS){
...
if (IsRunning()) {
      ...
      planner.buffer_segment(go[A_AXIS], go[B_AXIS], go[C_AXIS]
        ...
        if ((digitalRead(14)==HIGH&&digitalRead(15)==HIGH)||(digitalRead(14)==HIGH||digitalRead(15)==HIGH)){//Родион 06.11.20
      ...
      float go[MOV_AXIS] = { 0.0 },
        ...
        while(digitalRead(18) == LOW){
          ...
          SERIAL_PROTOCOLPAIR("BLTouch is: ", digitalRead(18));
...
void home_all_axes() { gcode_G28(true); }
  ...
  inline void gcode_G29() {
    ...
    #else // !PROBE_MANUALLY
    {
      ...
      // Возможно, где-то здесь есть код опускания щупа. STOW_PROBE();
...
//int32_t zyx, yzx;
//zyx=(int32_t) xzy;
//yzx=position(zyx);//синтаксис верный, ошибка скорее всего в типе переменных
SERIAL_PROTOCOLPAIR("ABCD:", xzy);
...
inline void gcode_M400() { planner.synchronize(); }
   ...
   * 
   * И снова Миша. 16.10.20. Запихали код щупа прямо в M401. Иначе конфликт пространств имен функций.
   * Работает.
   * 
   */
  inline void gcode_M401() {
    ...
    TOUCH_GOOD = digitalRead(TOUCH_SIGNAL_PIN);
   ...
   * 
   * Короче, функции STOW_PROBE нет. Поэтому код из нее выносится в функцию ниже (M402). Миша 16.10.20
   * 
   * Delay использовать нельзя. Выдает ошибку и перезагружается. Поэтому испольуем код из G4.
   * 
   */
...
inline void invalid_extruder_error(const uint8_t e) {
  ...
  void pe_set_magnet(const uint8_t extruder_num, const uint8_t state) {
    switch (extruder_num) {
      case 1: OUT_WRITE(SOL1_PIN, state); OUT_WRITE(SOL2_PIN, LOW); break;
